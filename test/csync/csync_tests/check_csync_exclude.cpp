...
static int setup_init(void **state) {
    ...
    excludedFiles->addExcludeExpr("*.💩"); // is this source file utf8 encoded?
    excludedFiles->addExcludeExpr("пятницы.*");
    excludedFiles->addExcludeExpr("*/*.out");
    excludedFiles->addExcludeExpr("latex*/*.run.xml");
    excludedFiles->addExcludeExpr("latex/*/*.tex.tmp");
...
{
    excludedFiles->addExcludeExpr("/tmp/check_csync1/*");
...
static void check_csync_excluded(void **)
{
    ...
    /* ? character */
    excludedFiles->addExcludeExpr("bond00?");
...
#ifndef _WIN32
    /* brackets */
    excludedFiles->addExcludeExpr("a [bc] d");
    excludedFiles->reloadExcludeFiles();
    ...
    /* escapes */
    excludedFiles->addExcludeExpr("\\a \\* \\?");
    excludedFiles->reloadExcludeFiles();
    assert_int_equal(check_file_full("\\a \\* \\?"), CSYNC_NOT_EXCLUDED);
...
static void check_csync_excluded_traversal(void **)
{
    ...
    excludedFiles->addExcludeExpr("/exclude");
    excludedFiles->reloadExcludeFiles();
    ...
    excludedFiles->addExcludeExpr("excl/");
    excludedFiles->reloadExcludeFiles();
    ...
    excludedFiles->addExcludeExpr("/excludepath/withsubdir");
    excludedFiles->reloadExcludeFiles();
