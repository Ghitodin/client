...
CSYNC_EXCLUDE_TYPE _csync_excluded_common(const QList<QByteArray> &excludes, const char *path, int filetype, bool check_leading_dirs)
{
    size_t i = 0;
...
{
    return ctx->exclude_traversal_fn(path, filetype);
}
...
void ExcludedFiles::addExcludeExpr(const QByteArray &expr)
{
